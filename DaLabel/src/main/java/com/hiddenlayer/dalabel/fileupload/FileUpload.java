...
public class FileUpload {
	...
	public void labellingFileUpload(HttpServletRequest req) {
		String route = req.getSession().getServletContext().getRealPath("resources/image/labeling_bundle/" + user.id 고민해봐야 할 문제); // 절대경로
//		MultipartRequest mr = null;
//		try {
...
//		}
		...
		try {												// 해당 경로에 .zip이 있어야 하는데, 즉, 톰캣 가상서버에 zip파일을 먼저 올린다음에 푸는 행위를 시작해야 함
			zis = new ZipInputStream(new BufferedInputStream(new FileInputStream(new File(route, "Desktop.zip")))); // 경로에 있는 .zip 파일을 선택
			ZipEntry ze = null;
			BufferedOutputStream bos = null;
			int len = 0, idx = 1;
			while ((ze = zis.getNextEntry()) != null) { // unzip 해야하는 다음 파일이 존재한다면(!=null) 실행
				if (ze.getName().endsWith(".jpg")) { 
					bos = new BufferedOutputStream(new FileOutputStream(route + idx++ + ".jpg")); // 해당 경로에, idx(숫자)++ 이름으로 된 .jpg를 생성
